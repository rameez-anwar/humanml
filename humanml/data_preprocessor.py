...
"""
Data Preprocessor Module for HumanML.
...
class DataPreprocessor:
    ...
    def fit_transform(self, X: pd.DataFrame, y: Optional[pd.Series] = None) -> pd.DataFrame:
        ...
        # Safety check: if no transformers were created, return the original data
        if not self.transformer.transformers:
            if self.verbose:
                print("Warning: No features were selected for preprocessing. Using original features.")
        ...
        # Safety check: if transformed data has no features, use original data
        if X_transformed_df.shape[1] == 0:
            if self.verbose:
                print("Warning: Preprocessing resulted in empty feature set. Using original features.")
    ...
    def _detect_feature_types(self, X: pd.DataFrame) -> None:
        ...
        # Safety check: ensure at least one feature is selected for preprocessing
        if (len(self.feature_types['numeric']) + len(self.feature_types['categorical']) + 
            len(self.feature_types['date']) + len(self.feature_types['text']) + 
            len(self.feature_types['binary'])) == 0:
            # If no features were detected, treat all columns as numeric
            if self.verbose:
                print("Warning: No valid features detected. Treating all columns as numeric.")
            self.feature_types["numeric"] = X.columns.tolist()
    ...
    def _create_preprocessing_pipeline(self) -> None:
        ...
        self.transformer = ColumnTransformer(
            ...
            remainder='passthrough'  # Changed from 'drop' to 'passthrough' to keep all features
        )
    ...
    def _get_feature_names_out(self) -> List[str]:
        ...
        if hasattr(self.transformer, 'get_feature_names_out'):
            try:
                ...
                # If get_feature_names_out fails, fall back to original feature names
                return self.preprocessing_results.get('feature_names_original', [])
            ...
            if name == 'numeric':
                ...
                for column in columns:
                    try:
                        ...
                        # If accessing categories fails, just use the column name
        ...
        # If no feature names were constructed, fall back to original feature names
        if not feature_names:
            feature_names = self.preprocessing_results.get('feature_names_original', [])
